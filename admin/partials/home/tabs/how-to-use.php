...
<p>
    Firstly, you'll need to connect your Swaptify account by adding your API Access Token. You can find your Token <a href="<?php echo(esc_url(Swaptify::$url)); ?>/account/api" target="_blank">here</a>.
</p>

<p>
    To connect your Swaptify account, add your API Access Token under <a href="<?php echo(esc_url(admin_url('admin.php?page=swaptify-configuration'))) ?>">Swaptify > Configuration</a>.
</p>
...
<p>
    You can create Segments and their Swaps directly from the Swaptify website (<a href="<?php echo(esc_url(Swaptify::$url)) ?>/segments/create" target="_blank"><?php echo(esc_html(Swaptify::$prettyUrl)) ?></a>), or you can create your Segments and Swaps using the Shortcode Generator feature of this plugin. The content you build with the Shortcode Generator will be automatically sent to Swaptify. We recommend starting with the Shortcode Generator because this interface will be familiar to WordPress users.
</p>
...
<p>
    You can update your Default Content under <a href="<?php echo(esc_url(admin_url('admin.php?page=swaptify-default-content'))) ?>">Swaptify > Default Content</a>.
</p>
...
<p>
    You can setup Visitor Types under <a href="<?php echo(esc_url(admin_url('admin.php?page=swaptify-visitor-types'))) ?>">Swaptify > Visitor Types</a>. You will also find code samples for how to trigger a Visitor Type.
</p>
...
<p>
    Rules are sets of conditions used to determine which Swaps to display. After you've created a Segment with Swaps, you can tell Swaptify which Swaps to show based on Rules that you can configure here: <a href="<?php echo(esc_url(Swaptify::$url)) ?>/rules" target="_blank"><?php echo(esc_url(Swaptify::$prettyUrl)) ?>/rules</a>. When there is a conflict between Visitor Types and Rules, Rules will override the Visitor Type. Be careful with your Rule logic so that you don't create conflicting personalization experiences.
...
<p>
    You can setup Events under <a href="<?php echo(esc_url(admin_url('admin.php?page=swaptify-event-settings'))) ?>">Swaptify > Event Settings</a>. You will also find code samples for how to trigger an Event.
</p>
